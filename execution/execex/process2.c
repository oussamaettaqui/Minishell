...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/08 17:01:45 by bchokri           #+#    #+#             */
/*   Updated: 2024/07/19 16:58:08 by bchokri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	help_process2(t_exec data)
{
	char	**commands;
	char	*path;
	char	**paths;
	...
	if (pid2 == 0)
	{
		...
		if (data.number_cmd > 1)
		{
			if (data.cmds_lst->fd_in != 0)
				...
				dup2(data.fds[data.id_pipe][0], data.cmds_lst->fd_in);
		}
		close_fds(data);
		help_process2(data);
